    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 429) {
          ...
          const errors = result.details.map((d: { message: string }) => d.message).join(", ");
    ...
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-white/10 bg-surface/60 px-8 py-16 text-center">
        ...
        <button
          ...
          className="mt-6 text-sm text-brand-bright hover:text-brand transition-colors"
        >
    ...
    return (
      <div className="rounded-2xl border border-white/10 bg-surface/60 p-7 sm:p-9">
        ...
        <p className="text-sm text-mist-dim mb-4">
          ...
          <a href={`mailto:${email}`} className="text-brand-bright hover:text-brand transition-colors">
            {email}
          </a>
          {" "}or call{" "}
          <a href={`tel:${phone}`} className="text-brand-bright hover:text-brand transition-colors">
            {phone}
          </a>
          .
        </p>
        <button
          ...
          className="text-sm text-brand-bright hover:text-brand transition-colors"
        >
          Try again
        </button>
      </div>
