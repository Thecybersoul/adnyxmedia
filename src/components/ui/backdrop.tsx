...
export function GlowOrbs({ className }: { className?: string }) {
  return (
    <div aria-hidden className={cn("pointer-events-none absolute inset-0 overflow-hidden", className)}>
      <div className="absolute -top-40 left-1/4 size-[36rem] rounded-full bg-brand/25 blur-[140px] animate-pulse-glow" />
      <div
        className="absolute top-1/3 right-0 size-[28rem] rounded-full bg-brand-bright/15 blur-[130px] animate-pulse-glow"
        style={{ animationDelay: "1.2s" }}
      />
      <div
        className="absolute bottom-0 left-1/3 size-[30rem] rounded-full bg-brand-deep/25 blur-[140px] animate-pulse-glow"
