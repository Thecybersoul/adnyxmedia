...
- **Supabase Storage** (`@supabase/supabase-js`) — image/video storage for the media library
...
1. **Create a Postgres database.** Any provider works (Supabase, Neon, RDS...) — set `DATABASE_URL` to its connection string.
2. **Set up Supabase Storage for uploads.** On your Supabase project's Settings → API page, copy the Project URL and the `anon` / `service_role` keys into `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, and `SUPABASE_SERVICE_ROLE_KEY`. The storage bucket is created automatically on first upload — no separate bucket-creation step.
...
4. **Copy `.env.example` to `.env.local`** and fill in the values above for local dev (`vercel env pull .env.local` will do this automatically once the env vars are set in your Vercel project).
...
From `/admin` you can edit: company info, the homepage hero (including swapping in a photo/video), the stats bar, services, the "how it works" steps, testimonials, the trusted-brands logos, About page values/timeline, and the full locations inventory (add/edit/delete sites, including per-site photos/videos). All uploads go through the Media Library, backed by Supabase Storage.
...
src/
  app/
    ...
    api/admin/              Supabase Storage signed-upload + media list API, used by the admin UI
  components/
...
2. **Billboard photos** — two ways to add them: drop files into `public/images/locations/<slug>.jpg` (see **[IMAGES.md](./IMAGES.md)** for naming/sizing), or upload per-site photos/videos from `/admin` (stored in Supabase Storage) — the admin upload takes priority when both are present.
...
3. Set `DATABASE_URL` (any Postgres provider), the three `SUPABASE_*` Storage env vars, and `ADMIN_PASSWORD` / `ADMIN_SESSION_SECRET` under Vercel's Environment Variables.
